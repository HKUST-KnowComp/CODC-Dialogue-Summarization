...
def main(opt):
    ...
    if opt.world_size > 1:
        ...
        if not opt.multi_proc_loading:
            single_main(opt, 0)
        else: 
            print('start a producer process')
            queues = []
            mp = torch.multiprocessing.get_context('spawn')
            ...
            q = mp.Queue(opt.queue_size)
            queues += [q]
            proc = mp.Process(target=run, args=(
                opt, device_id, error_queue, q, semaphore), daemon=True)
            proc.start()
            # logger.info(" Starting process pid: %d  " % procs[device_id].pid)
            # error_handler.add_child(procs[device_id].pid)
            error_handler.add_child(proc.pid)
            producer = mp.Process(target=batch_producer,
                                  args=(train_iter, queues, semaphore, opt,),
                                  daemon=True)
            producer.start()
...
def run(opt, device_id, error_queue, batch_queue, semaphore):
    """ run process """
    try:
        real_id = device_id if opt.device_id >= 0 else opt.gpu_ranks[device_id]
        gpu_rank = onmt.utils.distributed.multi_init(opt, real_id)

        if opt.device_id < 0:
            if gpu_rank != opt.gpu_ranks[device_id]:
                raise AssertionError("An error occurred in \
                      Distributed initialization")
        input_id = 0 if opt.device_id >= 0 else device_id
        single_main(opt, input_id, batch_queue, semaphore)
        ...
        import traceback
        if opt.device_id >= 0:
            error_queue.put((opt.device_id, traceback.format_exc()))
        else:
