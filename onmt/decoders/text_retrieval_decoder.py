...
class TextRetrievalDecoderBase(DecoderBase):
    """
    ...
    def __init__(self, 
        ...
        super(TextRetrievalDecoderBase, self).__init__()
    ...
    def forward(self, enc_state, memory_bank, tgt, neg_tgt, memory_lengths, tgt_length):
        """
        Args:
            enc_state ()
        ...
        # h_context = self.attend_hidden_states(self.attend_vec, memory_bank, memory_lengths) # different attention types
        # [batch_size, hidden_dim]

        tgt_emb = self.embeddings(tgt)
        pos_score = self.rank_score(enc_hidden, memory_bank, memory_lengths, tgt_emb, tgt_length)

        if neg_tgt is not None:
            neg_tgt_emb = self.embeddings(neg_tgt)
            neg_score = self.rank_score(enc_hidden, memory_bank, memory_lengths, neg_tgt_emb, tgt_length)
        else:
            neg_score = None
