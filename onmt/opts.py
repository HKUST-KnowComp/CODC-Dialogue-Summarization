...
def model_opts(parser):
    ...
    group.add('--encoder_type', '-encoder_type', type=str, default='rnn',
              choices=['rnn', 'brnn', 'mean', 'transformer', 'cnn', 'elmo', 'qa3'],
    ...
    group.add('--vocab_sample_method', '-vocab_sample_method', type=str, default="freq",
              choices=["freq", "uniform", "log"],
              help="Sample methods of negative samples")
    group.add('--score_output_function', '-score_output_function', type=str, default="None",
              choices=["relu", 'sigmoid', 'tanh', 'None'])
    group.add('--text_ret_decoder_attn', '-text_ret_decoder_attn', type=str, default="None",
              choices=["None", "global", "one"])

    group.add('--pairenc', '-pairenc', action="store_true",
              help="Whether to use pairenc or not")

    group.add('--knowledge', '-knowledge', action="store_true",
              help="Whether to use knowledge or not")
    group.add('--know_attn_type', '-know_attn_type',
              type=str, default=None,
              choices=['dot', 'general', 'mlp', 'none'],
              help="knowledge attention type")
    group.add('--know_attention_function', '-know_attention_function',
              type=str, default="softmax", choices=["softmax", "sparsemax"])

    # transformer knowledge
    group.add('--know_query', '-know_query',
              type=str, default="None", choices=["None", 
              'ori_query','ori_query_add_inputs','ori_query_add_inputs_norm'],
              help="deprecated!")
    group.add('--prob_logits_type', '-prob_logits_type',
              type=str, default="include_k_context", choices=["include_k_context", 
              'original'],
              help="which function to be used in p_vocab")
    group.add('--trans_know_query_type', '-trans_know_query_type',
              type=str, default="query_once", choices=["query_once", 
              "iter_query",], help="query_once means use the query in the top "
              "layer of decoder to attend to knowledge, iter_query means query "
              "iteratively by all layers of transformer")
    


    group.add('--input_attn', '-input_attn', action="store_true",
              help="Use encoder hidden states {hi} to attend to knowledge"
              " and get contexted knowledge vector each steps")
    
    group.add('--know_star_attn_share', '-know_star_attn_share', action="store_true",
              help="the attention to {k_star_i} is shared with the attention used "
              "when attending to {h_i}")

    
    # group.add('--know_attn_style', '-know_attn_style',
    #           type=str, default="traditional", choices=["traditional", "both", "max_hi"])

    group.add('--know_emb_init', '-know_emb_init',
              type=str, default="from_scratch", choices=["from_scratch", "from_tgt", "share_with_tgt", "share_with_src"])
    
    group.add('--know_loss_lambda', '-know_loss_lambda', type=float, default=-1,
              help="know_loss_lambda for loss copy")

    group.add('--p_kgen_loss', '-p_kgen_loss', action="store_true",
              help="p_kgen_loss")
    group.add('--p_kgen_func', '-p_kgen_func',
              type=str, default=None,
              choices=['linear', 'mlp'],
              help="knowledge attention type")
...
def preprocess_opts(parser):
    ...
    group.add('--elmo', '-elmo', action="store_true",
              help='test elmo')
    ...
    group.add('--train_know', '-train_know', required=False, nargs='+',
              help="Path(s) to the training knowledge data")
    ...
    group.add('--valid_know', '-valid_know',
              help="Path to the validation knowledge data")
    ...
    group.add('--tgt_vocab', '-tgt_vocab', default="",
              help="Path to an existing target vocabulary. Format: "
                   "one word per line.")
    group.add('--know_vocab', '-know_vocab', default="",
    ...
    group.add('--know_vocab_size', '-know_vocab_size', type=int, default=50000,
    ...
    group.add('--know_words_min_frequency',
              '-know_words_min_frequency', type=int, default=0)
    ...
    group.add('--know_seq_length', '-know_seq_length', type=int, default=50,
              help="Maximum knowledge sequence length to keep.")
    group.add('--know_seq_length_trunc', '-know_seq_length_trunc',
              type=int, default=None,
...
def train_opts(parser):
    ...
    group.add('--optim_seperate', '-optim_seperate', action="store_true",
              help="train elmo with seperate optimizer")

    group.add('--multi_proc_loading', '-multi_proc_loading', action="store_true",
              help="Use an external process to load data in single_train mode")
    ...
    group.add('--know_train_from', '-know_train_from', default='', type=str,
              help="Train from a pre-trained baseline model, "
                    "then add knowledge module")

    group.add('--know_train_from_type', '-know_train_from_type', default='none',
              choices=['finetune', 'fix_prev_model'],
    ...
    parser.add_argument("-lr_elmo", default=2e-3, type=float)
    ...
    parser.add_argument("-warmup_steps_elmo", default=8000, type=int)
    ...
    group.add('--image_channel_size', '-image_channel_size',
              ...
              help="Using grayscale image can training "
                   "model faster and smaller")
    group.add('--elmo_dropout', '-elmo_dropout', type=float, default=.00,
              help="elmp_dropout")
    group.add('--finetune', '-finetune', action="store_true",
              help="Whether to finetune elmo/bert")
...
def translate_opts(parser):
    ...
    group.add('--elmo', '-elmo', action="store_true",
              help="elmo")
    group.add('--imp_gen', '-imp_gen', action="store_true",
              help="imp_gen")
    group.add('--ret_num', '-ret_num', type=int, default=10,
              help="Number of retrieved words")
    ...
    group.add('--pairenc', '-pairenc', action='store_true',
              help="pairencoder")
    group.add('--decode_know_without_copy', '-decode_know_without_copy', 
              action='store_true',
              help="decode know without know-copy, used for comparison")
    ...
    group.add('--know', '-know',
              help='True knowledge sequence (used in knowledge mode)')
    ...
    group.add('--image_channel_size', '-image_channel_size',
              ...
              help="Using grayscale image can training "
                   "model faster and smaller")
    group.add('--show_know_copy', '-show_know_copy', action="store_true",
              help='check which of the decoded word is generated by know-copy mechanism')
