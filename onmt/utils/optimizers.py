...
class Optimizer(object):
    ...
    def __init__(self, method, _learning_rate, max_grad_norm,
        ...
        self.original_lr = _learning_rate
    ...
    def set_parameters(self, params):
        ...
        if self.method == 'sgd':
            self.optimizer = optim.SGD(self.params, lr=self._learning_rate)
        elif self.method == 'adagrad':
            self.optimizer = optim.Adagrad(self.params, lr=self._learning_rate)
            for group in self.optimizer.param_groups:
                for p in group['params']:
                    self.optimizer.state[p]['sum'] = self.optimizer\
                        .state[p]['sum'].fill_(self.adagrad_accum)
        elif self.method == 'adadelta':
            self.optimizer = optim.Adadelta(self.params, lr=self._learning_rate)
        elif self.method == 'adam':
            self.optimizer = optim.Adam(self.params, lr=self._learning_rate,
                                        betas=self.betas, eps=1e-9)
        else:
            raise RuntimeError("Invalid optim method: " + self.method)

    def _set_rate(self, _learning_rate):
        self._learning_rate = _learning_rate
        if self.method != 'sparseadam':
            self.optimizer.param_groups[0]['lr'] = self._learning_rate
        else:
            for op in self.optimizer.optimizers:
                op.param_groups[0]['lr'] = self._learning_rate
    ...
    def backward(self, loss):
        ...
        if self._fp16 == "amp":
            ...
            kwargs = {}
            if "update_master_grads" in fn_args(self.optimizer.backward):
        ...
        if self.decay_method == "noam":
            ...
            if self.start_decay:
                if ((self._step - self.start_decay_steps)
                   % self.decay_steps == 0):
                    self._learning_rate = self._learning_rate * self.lr_decay
...
class Optimizer_ori(object):
    ...
    def from_opt(cls, model, opt, checkpoint=None):
        ...
        if opt.train_from and checkpoint is not None:
            ...
            ckpt_state_dict = {}
            if isinstance(optim, list) and isinstance(optim[0], Optimizer_ori):
                # ??? by ftq
                optim = optim[0].state_dict()
                # this is due to the modification at model_saver
            if isinstance(optim, Optimizer_ori):  # Backward compatibility.
...
def build_optim(model, args, checkpoint):
    """ Build optimizer """
    if args.model_dtype == "fp16":
        if args.optim == "fusedadam":
            _fp16 = "legacy"
        else:
            _fp16 = "amp"
    else:
...
def build_optim_elmo(model, args, checkpoint):
    """ Build optimizer """
    ...
    if checkpoint is not None:
        ...
        optim = Optimizer(
            args.optim, args.lr_elmo, args.max_grad_norm,
            beta1=args.adam_beta1, beta2=args.adam_beta2,
            decay_method=args.decay_method,
            warmup_steps=args.warmup_steps_elmo,
            fp16=_fp16)

    params = [(n, p) for n, p in list(model.named_parameters()) if n.startswith('encoder.elmo')]
    optim.set_parameters(params)


    return optim

def build_optim_dec(model, args, checkpoint):
    """ Build optimizer """
    if args.model_dtype == "fp16":
        if args.optim == "fusedadam":
            _fp16 = "legacy"
    ...
    params = [(n, p) for n, p in list(model.named_parameters()) if not n.startswith('encoder.elmo')]
    optim.set_parameters(params)
