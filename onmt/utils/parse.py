...
class ArgumentParser(cfargparse.ArgumentParser):
    ...
    def update_model_opts(cls, model_opt):
        ...
        if model_opt.rnn_size > 0 and model_opt.encoder_type != 'elmo':
    ...
    def validate_model_opts(cls, model_opt):
        ...
        assert model_opt.encoder_type == 'elmo' or model_opt.model_type == 'audio' or same_size, \
    ...
    def validate_train_opts(cls, opt):
        ...
        # if opt.copy_attn and opt.knowledge:
        #     raise AssertionError("Can't do copy_attn and knowledge together")
        assert (opt.pairenc and opt.encoder_type=="qa3") or \
                (not opt.pairenc and not opt.encoder_type=="qa3"),\
                "when using qa3, you should specify pairenc"

    @classmethod
    def validate_translate_opts(cls, opt):
        if opt.beam_size != 1 and opt.random_sampling_topk != 1:
            raise ValueError('Can either do beam search OR random sampling.')
        if opt.decode_know_without_copy and not opt.knowledge:
            raise ValueError('Can not run decode_know_without_copy without using knowledge option')
    ...
    def validate_preprocess_args(cls, opt):
        ...
        if opt.pairenc:
            assert opt.src_seq_length_trunc is None ,\
                "don't trunc src seq when pair encoding"
