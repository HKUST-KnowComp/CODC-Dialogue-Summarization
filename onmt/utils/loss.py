...
def build_loss_compute(model, tgt_field, opt, train=True, special_tokens=[]):
    """
    ...
    if opt.copy_attn:
        ...
        criterion = onmt.modules.KnowGeneratorLoss(
            len(tgt_field.vocab), opt.copy_attn_force,
            unk_index=unk_idx, ignore_index=padding_idx
        )
    ...
    if opt.copy_attn:
        ...
        compute = onmt.modules.KnowGeneratorLossCompute(
            criterion, loss_gen, tgt_field.vocab, opt.copy_loss_by_seqlength,
            lambda_coverage=opt.lambda_coverage, know_loss_lambda=opt.know_loss_lambda,
            p_kgen_loss=opt.p_kgen_loss,
        )
    else:
...
class LossComputeBase(nn.Module):
    ...
    def __call__(self,
        ...
        # print('in LossComputeBase', shard_state['context_vecs'].size())
        if shard_size == 0:
    ...
    if eval_only:
        ...
        for k, (v, v_split) in non_none.items():
            ...
            if isinstance(v, torch.Tensor) and state[k].requires_grad:
                # print('require grad', k, [v_chunk.grad for v_chunk in v_split])
                # print('require grad 2', k, [v_chunk for v_chunk in v_split])
                variables.extend(zip(torch.split(state[k], shard_size),
                                     [v_chunk.grad for v_chunk in v_split]))
        inputs, grads = zip(*variables)
        # print('in shards', [item.size() for item in inputs], [test(item) for item in grads])
