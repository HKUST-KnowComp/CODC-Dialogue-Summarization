...
from onmt.modules.copy_generator import collapse_copy_scores, collapse_copy_scores_know
...
class Translator(object):
    ...
    def __init__(
            ...
            copy_attn=False,
            knowledge=False,
            decode_know_without_copy=False,
            global_scorer=None,
            out_file=None,
            report_score=True,
            logger=None,
        ...
        self._src_vocab = dict(self.fields)["src"].base_field.vocab
        ...
        self.decode_know_without_copy = decode_know_without_copy
    ...
    def from_opt(
        ...
        return cls(
            ...
            knowledge=model_opt.knowledge,
            decode_know_without_copy=opt.decode_know_without_copy,
    ...
    def translate(
        ...
        if know:
            feed_reader.append(self.know_reader)
            feed_data.append(("know", know))
            feed_dirs.append(None)
        ...
        for batch in data_iter:
            batch_data = self.translate_batch(
                batch, data.src_vocabs, data.know_vocabs, attn_debug
            )
            if self.imp_gen:
                for words in batch_data["predictions"]:
    ...
    def translate_batch(self, batch, src_vocabs, know_vocabs, attn_debug):
        """Translate a batch of sentences."""
        with torch.no_grad():
            if self.beam_size == 1 and not self.imp_gen:
    ...
    def _run_encoder(self, batch):
        ...
        if self.elmo:
            ...
            assert self._src_vocab is not None

            # check whether src_vocab is ok
            # elmo_src = [[self._src_vocab.itos[idx] for idx in src[:, b_s, 0] if idx >= 4] for b_s in range(batch_size)]
            elmo_src = [[self._src_vocab.itos[idx] for idx in src[:, b_s, 0]] for b_s in range(batch_size)]
        else:
            elmo_src=None

        if elmo_src is not None:
            enc_states, memory_bank, src_lengths = self.model.encoder(elmo_src, src_lengths, dev=self._dev)
        elif self.pairenc:
            enc_states, memory_bank, _ = self.model.encoder(src, q_lens_list, a_lens_list, src_lengths)
        else:
    ...
    def _decode_and_generate(
            ...
            batch_offset=None):
        if self.copy_attn:
            # Turn any copied words into UNKs. Why by ftq
        ...
        if self.model.input_attention is not None:
            print('in translator input attention')
            input_know_vecs, input_know_attns = self.model.input_attention(memory_bank, memory_lengths, know, know_length)
        ...
        dec_out, dec_attn = self.model.decoder(
            decoder_in, memory_bank, know=know, know_lengths=know_length,
        ...
        if self.copy_attn:
            ...
            attn = dec_attn["knowledge"]
            context_vecs = dec_attn["context_vecs"]
            scores, _ = self.model.generator(dec_out.view(-1, dec_out.size(2)),
                                          attn.view(-1, attn.size(2)),
                                          know_map,context_vecs.view(-1, context_vecs.size(2)))
            # here we have scores [tgt_lenxbatch, vocab] or [beamxbatch, vocab]
            if batch_offset is None:
                scores = scores.view(batch.batch_size, -1, scores.size(-1))
            else:
            ...
            scores = collapse_copy_scores_know(
                scores,
                batch,
                self._tgt_vocab,
                know_vocabs,
                batch_dim=0,
                batch_offset=batch_offset,
                decode_know_without_copy=self.decode_know_without_copy
            )
            scores = scores.view(decoder_in.size(0), -1, scores.size(-1))
            log_probs = scores.squeeze(0).log()
            # returns [(batch_size x beam_size) , vocab ] when 1 step
    ...
    def _translate_batch(
        ...
        know_map = (tile(batch.know_map, beam_size, dim=1)
                   if use_know_map else None)
        self.model.decoder.map_state(
            lambda state, dim: tile(state, beam_size, dim=dim)) # don't do changes

        if self.knowledge:
            know, know_length = batch.know
            if isinstance(know, tuple):
                know = tuple(tile(x, beam_size, dim=1) for x in know)
                mb_device = know[0].device
            else:
                know = tile(know, beam_size, dim=1)
                mb_device = know.device
            know_length = tile(know_length, beam_size)
        else:
            know = None
            know_length = None
        ...
        if self.imp_gen:
            ...
            # just decode one step and find the k words with the largest value
            log_probs, attn = self._decode_and_generate(
                decoder_input,
                memory_bank,
                know,
            ...
            # select from top k
            print('log_probs size', log_probs) # should be [batch_size, vocab_size]

            retrieved = torch.argsort(log_probs, dim=1, descending=True)[:, :self.ret_num]

            print('retrieved', retrieved)
        
            words = [[self._tgt_vocab.itos[i] for i in retrieved[b, :]] for b in range(batch_size)]
            print('words', words)
            results["predictions"] = words
        ...
        for step in range(max_length):
            ...
            if any_beam_is_finished:
                ...
                if self.knowledge:
                    if isinstance(know, tuple):
                        know = tuple(x.index_select(1, select_indices)
                                            for x in know)
                    else:
                        know = know.index_select(1, select_indices)
                    know_length = know_length.index_select(0, select_indices)
                    if know_map is not None:
                        know_map = know_map.index_select(1, select_indices)
                memory_lengths = memory_lengths.index_select(0, select_indices)
