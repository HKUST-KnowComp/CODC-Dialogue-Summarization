...
class BowGenTranslator(object):
    ...
    def __init__(
        ...
        self.tgt = torch.cuda.LongTensor([2 for i in range(1)]).contiguous().view([1, 1, 1])
    ...
    def _run_decoder(self, src, tgt, enc_state, memory_bank, src_lengths):
        ...
        dec_out, _ = self.model.decoder(tgt[0, :, :].unsqueeze(0), memory_bank, memory_lengths=src_lengths)
    ...
    def _translate_batch(
        ...
        dec_out = self._run_decoder(src, self.tgt.contiguous().expand([1, batch_size, 1]), enc_states, memory_bank, src_lengths)
        # the returned stuff are words

        scores_tgt_vocab = self.model.generator(dec_out.squeeze(0))
