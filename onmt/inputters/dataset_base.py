...
def get_qa_list(tok_list):
    s = " ".join(tok_list)
    tmp = [t.strip() for t in s.split("<q>") if t]
    tmp_2 = [item.split("</q> <a>") for item in tmp]
    tmp_3 = [t for item in tmp_2 for t in item]
    tmp_4 = [t.replace("</a>", "").strip() for t in tmp_3]
    q_list = tmp_4[::2]
    a_list = tmp_4[1::2]
...
def _dynamic_dict(example, src_field, tgt_field, pairenc=False):
    ...
    if pairenc:
        # print(src)
        q_list, a_list = get_qa_list(src)
        q_list = [t.strip().split(" ")[:Q_MAX_LEN] for
                  t in q_list] + [[]]*(10-len(q_list))
        a_list = [t.strip().split(" ")[:A_MAX_LEN] for
                  t in a_list] + [[]]*(10-len(a_list))
        
        q_lens_list = [len(t) for t in q_list]
        a_lens_list = [len(t) for t in a_list]
        
        # assert len(q_list) == 10 and len(a_list) == 10
        assert len(q_lens_list) == 10 and len(a_lens_list) == 10

        for i, q in enumerate(q_list):
            q_list[i] = padding_list(q, Q_MAX_LEN, PAD_WORD)
        for i, a in enumerate(a_list):
            a_list[i] = padding_list(a, A_MAX_LEN, PAD_WORD)

        words = []
        for i in range(10):
            for token in q_list[i]:
                words.append(token)
            for token in a_list[i]:
                words.append(token)

        example["q_lens"] = tuple(q_lens_list)
        example["a_lens"] = tuple(a_lens_list)
        example["src"] = words
        src = words
...
def _dynamic_dict_know(example, know_field, tgt_field):
    """Create copy-vocab and numericalize with it.
    ...
    Args:
        example (dict): An example dictionary with a ``"src"`` key and
            maybe a ``"tgt"`` key. (This argument changes in place!)
        know_field (torchtext.data.Field): Field object.
        tgt_field (torchtext.data.Field): Field object.

    Returns:
        torchtext.data.Vocab and ``example``, changed as described.
    """
    know = know_field.tokenize(example["know"])
    # make a small vocab containing just the tokens in the source sequence
    unk = know_field.unk_token
    pad = know_field.pad_token
    know_ex_vocab = Vocab(Counter(know), specials=[unk, pad])
    unk_idx = know_ex_vocab.stoi[unk]
    # Map source tokens to indices in the dynamic dict.
    know_map = torch.LongTensor([know_ex_vocab.stoi[w] for w in know])
    example["know_map"] = know_map
    example["know_ex_vocab"] = know_ex_vocab

    if "tgt" in example:
        tgt = tgt_field.tokenize(example["tgt"])
        mask = torch.LongTensor(
            [unk_idx] + [know_ex_vocab.stoi[w] for w in tgt] + [unk_idx])
        example["know_alignment"] = mask
    return know_ex_vocab, example
...
class Dataset(TorchtextDataset):
    ...
    Attributes:
        ...
        know_vocabs (List[torchtext.data.Vocab])
    """

    def __init__(self, fields, readers, data, dirs, sort_key,
                 filter_pred=None, pairenc=False):
        self.sort_key = sort_key
        can_copy = 'src_map' in fields and 'alignment' in fields
        know_copy = 'know_map' in fields and 'know_alignment' in fields
        # know_copy has higher priority
        ...
        for ex_dict in starmap(_join_dicts, zip(*read_iters)):
            if know_copy:
                ...
                know_ex_vocab, ex_dict = _dynamic_dict_know(
                    ex_dict, know_field.base_field, tgt_field.base_field)
                self.know_vocabs.append(know_ex_vocab)
            if can_copy:
