...
def make_know(data, vocab):
    know_size = max([t.size(0) for t in data])
    know_vocab_size = max([t.max() for t in data if t.size(0) > 0]) + 1
    alignment = torch.zeros(know_size, len(data), know_vocab_size)
    for i, sent in enumerate(data):
        for j, t in enumerate(sent):
            alignment[j, i, t] = 1 # k_len, batch, know_vocab_size
    return alignment
...
def get_fields(
    ...
    if knowledge:
        know_field_kwargs = {"n_feats": n_know_feats,
                            "include_lengths": True,
                            "pad": pad, "bos": None, "eos": None,
                            "truncate": know_truncate,
                            "base_name": "know"}
        fields["know"] = fields_getters["text"](**know_field_kwargs)

    indices = Field(use_vocab=False, dtype=torch.long, sequential=False)
    fields["indices"] = indices

    if pairenc:
        fields["q_lens"] = torchtext.data.Field(
            use_vocab=False, dtype=torch.float,
            sequential=False)
        fields["a_lens"] = torchtext.data.Field(
            use_vocab=False, dtype=torch.float,
            sequential=False)
    ...
    if dynamic_dict:
        if knowledge:
            know_map = Field(
                use_vocab=False, dtype=torch.float,
                postprocessing=make_know, sequential=False)
            fields["know_map"] = know_map
            know_ex_vocab = RawField()
            fields["know_ex_vocab"] = know_ex_vocab
            know_align = Field(
                use_vocab=False, dtype=torch.long,
                postprocessing=make_tgt, sequential=False)
            fields["know_alignment"] = know_align
        # else:
        src_map = Field(
            use_vocab=False, dtype=torch.float,
...
def filter_example(ex, use_src_len=True, use_tgt_len=True, use_know_len=True,
                   min_src_len=1, max_src_len=float('inf'),
                   min_tgt_len=1, max_tgt_len=float('inf'),
                   min_know_len=1, max_know_len=float('inf')):
...
def _build_fields_vocab(fields, counters, data_type, share_vocab,
                        ...
                        know_vocab_size, know_words_min_frequency,
                        knowledge):
    ...
    if knowledge:
        build_fv_args["know"] = dict(
        max_size=know_vocab_size, min_freq=know_words_min_frequency)
    ...
    if data_type == 'text':
        ...
        if share_vocab:
            ...
            logger.info(" * merging src tgt and know vocab...")
            src_field = src_multifield.base_field
            tgt_field = tgt_multifield.base_field
            if knowledge:
                know_field = know_multifield.base_field
            else:
                know_field = None
            _merge_field_vocabs(
                src_field, tgt_field, know_field=know_field, vocab_size=src_vocab_size,
...
def _merge_field_vocabs(src_field, tgt_field, vocab_size, min_freq,
                        vocab_size_multiple, know_field=None):
    # in the long run, shouldn't it be possible to do this by calling
    # build_vocab with both the src and tgt data?
    specials = [tgt_field.unk_token, tgt_field.pad_token,
                tgt_field.init_token, tgt_field.eos_token]
    if know_field is not None:
        merged = sum(
            [src_field.vocab.freqs, tgt_field.vocab.freqs, know_field.vocab.freqs], Counter()
        )
    else:
        merged = sum(
    ...
    if know_field is not None:
        know_field.vocab = merged_vocab
        assert len(src_field.vocab) == len(know_field.vocab)
    assert len(src_field.vocab) == len(tgt_field.vocab) #  and \
...
def max_tok_len(new, count, sofar):
    ...
    global max_src_in_batch, max_tgt_in_batch, max_know_in_batch  # this is a hack
    ...
    # Know: [<bos> w1 ... wN <eos>]
    # max_know_in_batch = max(max_know_in_batch, len(new.know[0] + 0))
    src_elements = count * max_src_in_batch
    tgt_elements = count * max_tgt_in_batch
    # know_elements = count * max_know_in_batch
    # return max(src_elements, tgt_elements, know_elements)
    return max(src_elements, tgt_elements) # ftq
